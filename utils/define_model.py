...
def get_unet(minimum_kernel=32, do=0, activation=ReLU, iteration=1):
    ...
    up6 = concatenate([Conv2DTranspose(minimum_kernel*8, (2, 2), strides=(2, 2), padding='same')(conv5), conv4], axis=3)
    ...
    up7 = concatenate([Conv2DTranspose(minimum_kernel*4, (2, 2), strides=(2, 2), padding='same')(conv6), conv3], axis=3)
    ...
    up8 = concatenate([Conv2DTranspose(minimum_kernel*2, (2, 2), strides=(2, 2), padding='same')(conv7), conv2], axis=3)
...
#                 layer.name = f'model1{iteration_id+1}_{layer.name}_{count}'
                count += 1
...
#         conv1 = add(a_layers)
        ...
        #conv2 = concatenate([b, conv2], axis=3)
